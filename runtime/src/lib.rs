...
pub const VERSION: RuntimeVersion = RuntimeVersion {
        spec_name: create_runtime_str!("lunes-nightly"),
        impl_name: create_runtime_str!("lunes-nightly"),
        authoring_version: 1,
...
parameter_types! {
        /// Parcela (%) das taxas + gorjetas destinada ao autor do bloco (nó) como incentivo.
        pub const AuthorFeeShare: u32 = 75;
        /// Parcela (%) das taxas + gorjetas destinada ao tesouro/queima (deflação).
        pub const TreasuryBurnFeeShare: u32 = 25;
        /// Divisão interna (%) da parcela `TreasuryBurnFeeShare` que vai para o tesouro.
        pub const TreasuryHalfShare: u32 = 50;
        /// Divisão interna (%) da parcela `TreasuryBurnFeeShare` que é queimada.
        pub const BurnHalfShare: u32 = 50;
        /// Enquanto a emissão total estiver acima deste limite, metade da parcela de
        /// tesouro/queima é efetivamente **queimada** (mecanismo deflacionário). Abaixo
        /// dele, toda a parcela vai ao tesouro (preserva emissão nas fases iniciais).
        pub const DeflationIssuanceThreshold: Balance = 50_000_000 * UNIT;
}

/// Destinação das taxas de transação (`fees`) e gorjetas (`tips`):
///
/// 1. `fees` e `tips` são unificados em um único imbalance.
/// 2. `AuthorFeeShare`% vão para o autor do bloco atual (incentivo ao validador).
/// 3. Os `TreasuryBurnFeeShare`% restantes:
///    - Se a emissão total > `DeflationIssuanceThreshold`: metade vai ao tesouro e
///      metade é **queimada** (descartar o `NegativeImbalance` reduz a emissão total).
///    - Caso contrário: tudo vai ao tesouro.
///
/// A queima é feita simplesmente descartando (`drop`) o `NegativeImbalance`, o que
/// reduz a emissão total registrada pelo `pallet_balances`. Isso corrige o
/// comportamento anterior, que chamava `Balances::burn` sobre o valor já contido no
/// imbalance descartado (dupla contagem).
pub struct DealWithFees;
impl OnUnbalanced<NegativeImbalance> for DealWithFees {
        fn on_unbalanceds<B>(mut fees_then_tips: impl Iterator<Item = NegativeImbalance>) {
                if let Some(mut fees) = fees_then_tips.next() {
                        // Unifica gorjetas às taxas, se houver.
                        if let Some(tips) = fees_then_tips.next() {
                                tips.merge_into(&mut fees);
                        }

                        // Divide entre tesouro/queima e autor do bloco.
                        let (to_treasury_burn, to_author) =
                                fees.ration(TreasuryBurnFeeShare::get(), AuthorFeeShare::get());

                        // Incentivo ao autor do bloco (se não houver autor, o imbalance é queimado).
                        Author::on_unbalanced(to_author);

                        // Mecanismo deflacionário condicionado à emissão total.
                        if Balances::total_issuance() > DeflationIssuanceThreshold::get() {
                                let (to_treasury, to_burn) =
                                        to_treasury_burn.ration(TreasuryHalfShare::get(), BurnHalfShare::get());
                                TreasuryLunes::on_unbalanced(to_treasury);
                                // Queima: descartar o imbalance reduz a emissão total.
                                drop(to_burn);
                        } else {
                                TreasuryLunes::on_unbalanced(to_treasury_burn);
                        }
                }
        }
}
...
parameter_types! {
        ...
        pub const SignedDepositByte: Balance = 1 * NANOUNIT;
...
parameter_types! {
        pub const SessionPeriod: u32 = 30;
...
parameter_types! {
        ...
        // One cent: $10,000 / MB
        pub const PreimageByteDeposit: Balance = 1 * NANOUNIT;
}
...
parameter_types! {
        ...
        pub const DataDepositPerByte: Balance = 1 * NANOUNIT;
...
#[cfg(test)]
mod deal_with_fees_tests {
        use super::*;
        use frame_support::traits::OnUnbalanced;
        use sp_runtime::BuildStorage;

        /// Constrói um ambiente de teste com o genesis padrão do runtime.
        fn new_test_ext() -> sp_io::TestExternalities {
                let t = frame_system::GenesisConfig::default()
                        .build_storage::<Runtime>()
                        .expect("genesis padrão deve construir o storage");
                sp_io::TestExternalities::new(t)
        }

        /// Conta de tesouro usada por `TreasuryLunes` (mesmo endereço codificado no runtime).
        fn treasury_account() -> AccountId {
                hex!["2c11d2aff81147e5522539c51c1cb87bae94a0865d214f3983f3557a6732f26a"].into()
        }

        #[test]
        fn deal_with_fees_handles_empty_iterator() {
                new_test_ext().execute_with(|| {
                        // Não deve entrar em pânico com um iterador vazio.
                        DealWithFees::on_unbalanceds(core::iter::empty::<NegativeImbalance>());
                });
        }

        #[test]
        fn deal_with_fees_handles_zero_imbalance() {
                new_test_ext().execute_with(|| {
                        let zero = <Balances as FrameCurrency<AccountId>>::issue(0);
                        DealWithFees::on_unbalanceds(core::iter::once(zero));
                        // A emissão total permanece zero.
                        assert_eq!(Balances::total_issuance(), 0);
                });
        }

        #[test]
        fn deal_with_fees_routes_treasury_share_below_threshold() {
                new_test_ext().execute_with(|| {
                        // Emite um imbalance de taxa. Como não há autor de bloco no ambiente de
                        // teste, a parcela do autor é descartada (queimada). Abaixo do limiar de
                        // deflação, toda a parcela de tesouro/queima é creditada ao tesouro.
                        let fee_amount: Balance = 1_000_000;
                        assert!(Balances::total_issuance() <= DeflationIssuanceThreshold::get());

                        let fee = <Balances as FrameCurrency<AccountId>>::issue(fee_amount);
                        DealWithFees::on_unbalanceds(core::iter::once(fee));

                        // O tesouro deve receber exatamente `TreasuryBurnFeeShare`% da taxa.
                        let expected_treasury =
                                fee_amount / 100 * TreasuryBurnFeeShare::get() as Balance;
                        assert_eq!(
                                <Balances as FrameCurrency<AccountId>>::free_balance(&treasury_account()),
                                expected_treasury
                        );
                });
        }
        ...
        #[test]
        fn deal_with_fees_merges_tips_into_fees() {
                new_test_ext().execute_with(|| {
                        // Duas parcelas (taxa + gorjeta) devem ser unificadas antes do rateio.
                        let fee = <Balances as FrameCurrency<AccountId>>::issue(600_000);
                        let tip = <Balances as FrameCurrency<AccountId>>::issue(400_000);
                        DealWithFees::on_unbalanceds(vec![fee, tip].into_iter());

                        // Total combinado = 1_000_000; tesouro recebe 25% = 250_000.
                        let expected_treasury = 1_000_000u128 / 100 * TreasuryBurnFeeShare::get() as Balance;
                        assert_eq!(
                                <Balances as FrameCurrency<AccountId>>::free_balance(&treasury_account()),
                                expected_treasury
                        );
        ...
        #[test]
        fn fee_shares_are_internally_consistent() {
                // As parcelas configuradas devem sempre somar 100%.
                assert_eq!(AuthorFeeShare::get() + TreasuryBurnFeeShare::get(), 100);
                assert_eq!(TreasuryHalfShare::get() + BurnHalfShare::get(), 100);
        }
}
